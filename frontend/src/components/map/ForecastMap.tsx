...
export function ForecastMap({ selectedId, onSelectLocation }: ForecastMapProps) {
  ...
  const handleHimawariBandChange = useCallback((mode: HimawariBandMode) => {
    setHimawariBandMode(mode)
  ...
  return (
    <div className="relative w-full h-full" style={{ background: '#000000' }}>
      ...
      <MapControls
        ...
        tileTimestamp={tileTimestamp}
        tileStale={tileStale}
        tileError={tileError}
        himawariActiveBand={himawariActiveBand}
        himawariBandMode={himawariBandMode}
        onHimawariBandChange={handleHimawariBandChange}
      />
    </div>
  )
}
